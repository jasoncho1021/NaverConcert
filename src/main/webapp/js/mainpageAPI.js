document.addEventListener("DOMContentLoaded", function() {
 sendAJAX('/resvMng/products?start=0', makeList);
 sendAJAX('/resvMng/categories', getCategories);
 sendAJAX('/resvMng/promotions', makePromoList);
 ...
 bttn.addEventListener("click", function(evt) {
	 ...
	 if(categories == 0) {
		 sendAJAX('/resvMng/products?start='+ start, makeList);
	 } else {
		 sendAJAX('/resvMng/products?categoryId='+ categories + '&start=' + start, makeList);
	 }
 });
 ...
 tabmenu.addEventListener('click', function(evt) {
	 ...
	 if( el != null ) {
		 ...
		 if(categories == 0) {
			 ...
			 sendAJAX('/resvMng/products?categoryId='+ categories + '&start=0', makeList);
...
function makeList(val) {
	...
	var resultHTML = ["",""];
	var ul = document.querySelectorAll('.lst_event_box');			
	var productLimitList = val.productLimitList;
